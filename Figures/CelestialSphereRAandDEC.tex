...
\begin{tikzpicture}[tdplot_main_coords] % "THE GLOBE" showcase
    ...
    \LatitudePlane[equator]{\angEl}{0}
     ...
     \fill[ball color=white, fill opacity=1] (0,0) circle (\R);
    \draw (0,0) circle (\R);
    \coordinate (O) at (0,0);
...
%		\coordinate (equator) at (\H,0);
    ...
    \draw[->] (0,-\H-4) -- (0,\R+4) node[above] {}; %axis of rotation
%		\draw[->,rotate=-30.0] (0,-\H-5) -- (0,\R+5) node[above] {\bf{Zenith}}; %axis of rotation

    \path[xzplane] (\R,0) coordinate (XE);

%		\DrawLatitudeCircle[\R]{0} % equator
%		\DrawLatitudeCircle[\R,rotate=39,color=red]{0}
...
\shade[tdplot_screen_coords, ball color = red] (\tdplotresx,\tdplotresy) circle (\R);                                                                   % sphere
    ...
    \filldraw[ball color=white, fill opacity=0.1] (0,0) circle (\R);
    \draw (0,0) circle (\R);
...
%		\draw 
		\DrawLatitudeCircleName[\R,rotate=23.5, color=orange]{20}{ecl} % ecliptic
       % \LatitudePlane[equator]{50}{0}
...
%		September 23rd} (auteq);
		...
		%\node[below=1.0em,right=0.1em] at (vereq) {$\aries$};
