...
\bottomrule
\end{longtable}		
\caption*{\textbf{Notes}: The fitted exposure time is estimated from the best fit between the expected number of photons from the catalog stars and the number of counts measured on the sensor. A good agreement between actual exposure time and fitted exposure time further ensures that we are looking at the correct region of the sky (the matching algorithm does not use the fluxes of each star to determine the attitude solution, so the fitted flux is simply an extra protection against false positives. Although we only show small number statistics here, we have experimented very robust results in the lab over many clear nights using a \SI{125}{\milli\second} exposure. The exposure time is a parameter that can be set remotely and changed during flight if necessary.}																					
\end{table}																							
