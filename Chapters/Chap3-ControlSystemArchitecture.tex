...
\begin{figure}[!ht]
		\centering
		\includegraphics[width=0.7\textwidth]{Figures/FOGs.jpg} 
		\caption[Fiber optic gyro 3-axes mount]{Three single-axis Fiber Optic Gyroscopes are mounted in an orthogonal assembly and attached to the truss.}
		\label{fig:GyroMount}
\end{figure}
...
\caption*{\textbf{Notes}: Properties of the gyroscopes determined from the Allan variance analysis on an \SI{8}{\hour} sample with a sampling rate at \SI{100}{\hertz}. Note that \SI{1}{\deg\per\hour} =  \SI{1}{\arcsec\per\second}, and the Earth rotates at about \SI{15}{\arcsec\per\second} about the line joining the two poles.}
\end{table}
