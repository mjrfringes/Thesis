...
A diagram showing the flow of the main loop inthe control software is shown in Fig.~\ref{fig:SoftwareFlow}. 
\begin{figure}[!h]
	\centering
...
This loop is operated at a nominal frequency of \SI{100}{\hertz}, which is the speed at which we read out the gyroscopes and issue new commands to the control system. The star camera, as well as other absolute sensors are being processed by the Real time software, and appropriately propagated since they often correspond to a measurement that was taken some number of loops ago. Using a robust synchronization scheme slaved to our system's master clock, aligning the various pieces of information is possible with very high accuracy.
