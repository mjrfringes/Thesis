...
Because star-forming clusters are surrounded by interstellar matter from the surrounding molecular cloud, they usually cannot be studied at optical wavelengths, due to the large obscuration from dust grains in the cluster and the parent molecular cloud. Infrared observations can be used to probe these structures since the dust can acquire sufficient temperature to emit thermally from the mid-infrared to millimeter and radio wavelengths. 

The high density of YSOs within clusters, combined with their typical distances of few hundreds of parsecs requires a high angular resolution in order to capture the relevant spatial scales at which accretion mechanisms are happening throughout the YSO's life. [Millimeter-wave and radio observations can provide ..., but]
...
Virial theorem: the gravitational potential energy is equal to twice the internal thermal energy (applies to stable systems)
...
The impact of rotation need also be taken into account. As the core contracts, the centrifugal force can provide some support slowing down the collapse of the star. However, even starting with very small core rotation veolcities, conservation of angular momentum would make the gas reach very high velocities as it infalls to make the star, which would contradict the observations showing that stars actually have relatively small amounts of angular momentum \cite{McKee:2007bd}. The impact of magnetic fields and turbulence inside the core are often invoked in an attempt to address this problem. Some models of slowly-rotating infalling clouds suggest more complex density profiles for the envelopes \citep[e.g.][]{Ulrich:1976ho,Terebey:1984hi} than simple power laws, but are observationally difficult to constrain.

Once the star is born and the cloud contracts, some of the surrounding material which is bound to the new YSO naturally flattens into a disk as it is accreted onto the star. The object now has three characteristic  components: the star itself; the flattened disk; and a diffuse envelope which constitutes the mass reservoir for future accretion onto the star.
...
We have determined that YSOs are composed of a star, a disk, and an envelope. The star is believe to be fairly well understood as a young object in hydrostatic equilibrium on its way to the main sequence. Depending on many parameters, the spatial distribution of gas in the disk and the envelope can be predicted by simple models, but in all likelihood is very complex, inhomogeneous, and asymmetric. Often, the envelope is thought to open up a cavity around the poles of the rotating system, where material can escape the system (through outflows, for example [REFERENCE?]). For clarity, we will discuss here the simple models that can be used to describe the YSOs in the multiple stages of their evolution.

In the most common model of the evolution of stars, the first stage consists of a dense core right after the YSO is born. The disk is almost inexistent, the envelope still is dense and circularly symmetric. This is called Stage 0. As the gas falls onto the star, rotation of the entire system tends to clear out a cavity at both poles of the system. As the system evolves, the cavity opening angle grows, the density of the envelope decreases, and the size of the disk increases. When a YSO is Stage III, the envelope is almost entirely depleted.

The various stages of YSO (from 0 to III) have very distinct observational signatures, although are highly dependent on the viewing angle. In Stage 0, the signature consists almost exclusively of thermal emission from the envelope which is heated by the central object. This is representative of a single blackbody-like peak in the far-IR, which peaks around \SI{100}{\micro\meter}. In Stage I, as the density lowers, light from the central star can scatter off the dust grains in the system and escape the system. This adds emission below \SI{10}{\micro\meter} which is highly dependent of the geometry of the system and the scattering properties of the dust [REFER TO APPROPRIATE DUST SECTION].



\subsubsection{Mass functions and accretion mechanisms}

\paragraph{Accretion mechanisms}

The accretion mechanism most commonly consists of Bondi accretion [REFERENCE], and represents the transfer of mass from the envelope or the disk to the star. As the YSO moves through its parent cloud, it can also accrete more mass throughout its life that was not originally part of the original condensation that initiated the collapse (this mechanism is referred to as Bondi-Hoyle accretion).

Various models exist for accretion, from a quasi-static slow process \citep{Shu:1977ef} to a very rapid and brutal process \citep[e.g.][]{Larson:1967ee}. \cite{McKee:2007bd} suggest that the infall process is most likely slowing down as a function of time, which complicates its observational validation. Most models result in an infalling envelope with density profiles which follow laws from $r^{-1/2}$ to $r^{-2}$, an important observable that can be useful to test these theories.
...
\paragraph{Protostellar luminosity function}
...
Note that most of the mass is not in the dust

Dust is a tracer of the gravitational potential, hence the mass.


\subsubsection{Dust populations and properties}

The composition, structure, and size distribution of interstellar grains is the result of injection of dust from stellar outflows into the interstellar medium, followed by destruction, growth, coagulation, and photoprocessing of interstellar grains.
...
BETTII is expected to launch from Fort Sumner, NM, for its first engineering flight. After a morning launch, we expect to wait until nightfall to achieve proper thermal stabilization and achieve our science goals. We expect the flight to last about \SI{16}{\hour}, although this is highly dependent on the weather.
