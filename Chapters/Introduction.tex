...
A considerable amount of literature exists on star formation and the various physical processes involved in forming stars. In this section, we review some of the most standard views of how stars grow to acquire their final masses.
...
Various models for infall exist, from a quasi-static slow process \citep{Shu:1977ef} to a very rapid and brutal process \citep[e.g.][]{Larson:1967ee}. \cite{McKee:2007bd} suggest that the infall process is most likely slowing down as a function of time, which complicates its observational validation. Most models result in an infalling envelope with density profiles which follow laws from $r^{-1/2}$ to $r^{-2}$, an important observable that can be useful to test these theories.

The impact of rotation need also be taken into account, since as the core contracts, conservation of angular momentum could make the gas reach very high velocities, which would contradict the observations \cite{McKee:2007bd}. To solve this problem, limitations to the models are usually invoked, such as the impact of magnetic fields and turbulence inside the core itself, which are complex to model. The exact structure


One typical envelope density profile was popularized by \cite{Ulrich:1976ho}
...
\paragraph{Young Stellar Objects classification}
