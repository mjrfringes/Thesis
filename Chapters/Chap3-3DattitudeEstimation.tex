...
\begin{equations}
\sigma_\gyro\units{rad.s$^{-0.5}$} = \frac{\pi}{60\times180}\times \textrm{ARW}\units{deg.hr$^{-0.5}$} \sim 1.5\times 10^{-7}~\textrm{rad}.\textrm{s}^{-0.5}.
\end{equations}

Note that we can relate the ARW to the measured discrete rate noise uncertainty $\sigma(\nGyros^\textrm{meas})$ with:
\begin{equations}
\sigma(\nGyros^\textrm{meas})\units{deg.s$^{-1}$} = \textrm{ARW}\units{deg.hr$^{-0.5}$}\times 60\sqrt{\textrm{BW}\units{Hz}}
\end{equations}
where $\textrm{BW}\units{Hz}$ is the gyroscope's bandwidth, equal to 50~Hz for our system. We obtain a quantity close to the measured quantity, $\sigma(\nGyros^\textrm{meas})\sim 0.2$~arcsec.s$^{-1}$.
...
\begin{equations}
\sigma_\bias\units{rad.s$^{-3/2}$} = 0.005~\textrm{deg.hr}^{-1}\times \sqrt{\textrm{BW}\units{Hz}}  \sim 1.8\times 10^{-7}~\textrm{rad}.\textrm{s}^{-3/2}.
\end{equations}
