...
The attitude estimation consists of combining high-frequency angular velocity measurements of the payload with low-frequency attitude measurements. The high-frequency measurements, usually from gyroscopes, are relative measurements, and exhibit biases. The attitude measurements are absolute. The Kalman filter \citep{Kalman:1960ii} combines these two types of measurements in a mathematical formalism that uses a model of the physical relationship between them. The goal of the filter is to estimate the bias of the high-frequency measurements, hence providing bias-corrected, drift-less, trustworthy dynamical information that can be used to estimate the attitude at all times, even when there is no absolute measurement.
...
The angular random walk (ARW) that we measure is ARW~$\sim$\SI{5e-4}{\deg\per\hour}. This means that if we integrate the gyroscope's rate for 1~hour, the $1\sigma$ uncertainty on our position would be 
...
\end{equations}
We now have $\gyroVec^\textrm{meas} = (\bI_{3\times 3} + \textbf{C}\gyroVec^\textrm{true}$, so the gyro error introduced by the misalignment is $\Delta\gyroVec^\textrm{meas}=\textbf{C})\gyroVec^\textrm{true}$. The additional state components are $\textbf{c} = \begin{bmatrix} c_{xy}& c_{xz} & c_{yz}\end{bmatrix} ^T$, and we can rearrange the matrix terms to write: $\gyroVec^\textrm{true} =\boldsymbol{\Omega}_c \textbf{c}$, with $\boldsymbol{\Omega}_c = \begin{bmatrix}  \end{bmatrix}$. Similarly to the error-representation equations in the Kalman filter model expressed in the previous sections, we then obtain the new upper right block of the transition matrix:
\begin{equation}
...
\begin{equations}
\Delta\gyroVec^\textrm{meas}_k = \boldsymbol{\Omega}_k\gyroVec^\textrm{true},
\Delta\gyroVec^\textrm{meas}_m = \boldsymbol{\Omega}_m\gyroVec^\textrm{true},
\end{equations}
for a total error in velocity $\Delta\gyroVec^\textrm{meas} = \Delta\gyroVec^\textrm{meas}_k + \Delta\gyroVec^\textrm{meas}_m + \Delta\gyroVec^\textrm{meas}_\bias$ if we also include the bias that we discussed in our standard estimator. We have here:
\begin{equations}
